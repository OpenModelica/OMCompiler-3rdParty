...
/*          If STOREV = 'C', LDV >= lmax(1,N); if STOREV = 'R', LDV >= K. */
...
/*              T(1:i-1,i) := T(1:i-1,1:i-1) * T(1:i-1,i) */
		...
		if (i__ > 1) {
		    prevlastv = lmax(prevlastv,lastv);
		} else {
		    prevlastv = lastv;
		}
	    }
...
/*                 T(i+1:k,i) := T(i+1:k,i+1:k) * T(i+1:k,i) */
		    ...
		    ztrmv_("Lower", "No transpose", "Non-unit", &i__1, &t[i__ 
			    ...
			    ;
		    if (i__ > 1) {
			prevlastv = lmin(prevlastv,lastv);
		    } else {
			prevlastv = lastv;
		    }
		}
